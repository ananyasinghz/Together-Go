...
| Quality / build | pytest, pytest-cov, httpx (TestClient), mutmut 2.x (mutation on `auth_utils` only), Git, GitHub Actions ([`.github/workflows/ci.yml`](.github/workflows/ci.yml)), Docker / Docker Compose |
...
| Docker stack | `docker-compose.yml`, `backend/Dockerfile`, `frontend/Dockerfile`, `backend/requirements-docker.txt` |
...
4. **Docker (optional):** from the repository root, run `docker compose up --build`. This starts MongoDB, the API on port 8000, and the built SPA (nginx) on port 3000. See [`README.md`](README.md).
